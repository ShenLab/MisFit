...
def main():
	...
	for batch_index in range(0, len(data), batch_size):
		...
		with torch.no_grad():
			results = model(batch_tokens, repr_layers=[33], return_contacts=True, need_head_weights=False)

#		representation = results["representations"][33].cpu().numpy()
...
#		ref = np.take_along_axis(logits, np.expand_dims(batch_tokens.cpu().numpy(), -1), -1)
#		logits = np.take(logits, indices, axis = 2) # B * (MAX_L + 2) * 20
#		logits = ref - logits

		for i, label in enumerate(batch_labels):
#			np.save(f"repr_{MAX_L}_{OVERLAP}/{label}.npy", representation[i, 1:(batch_l[i]+1), :]) # (L, 1280)
			np.save(f"contact_{MAX_L}_{OVERLAP}/{label}.npy", contacts[i, :batch_l[i], :batch_l[i]]) # (L, L)
#			np.save(f"logits_{MAX_L}_{OVERLAP}/{label}.npy", logits[i, 1:(batch_l[i]+1), :]) # (L, 20)
