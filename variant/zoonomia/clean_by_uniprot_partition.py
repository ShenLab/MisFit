...
def process_one_id(id0, seq_file, MSA_file, out_file): # by ensembl_protein_id
	records_list = []
...
def main(filename, start, end):
	...
	MSA_dir = "MSA_by_transcript/"
	all_filenames = os.listdir(MSA_dir)
	for filename in all_filenames[start:end]:
		filename_split = filename.split(".")
		gene_name = filename_split[1]
		row = geneset_df[geneset_df['Symbol']==gene_name]
		if len(row) == 0:
			continue
		_, row = next(row.iterrows())
		uniprot_id = row['UniprotID']
		MSA_file = MSA_dir + filename
		seq_file = "uniprot_seq/" + uniprot_id + ".fasta.gz"
		out_file = out_dir + uniprot_id + "_aligned.fasta.gz"
		try:
			depth = process_one_id(uniprot_id, seq_file, MSA_file, out_file)
		except:
