...
def main():
	...
	for _, row in geneset.iterrows():
		...
		filename = f"mis_info_by_protein_mapping/{uniprot_id}_info.txt.gz"
		if exists(filename):
			...
			df = df[(df['outlier']==False)&(df['LCR']==False)&(df['mu']>0)&(df['AN']>0)][['AN', 'AC', 'mu']]
			if len(df) > 0:
				df.to_csv(f"mis_ukbb_gnomad/{uniprot_id}_info.txt.gz", sep = "\t", index = False)
