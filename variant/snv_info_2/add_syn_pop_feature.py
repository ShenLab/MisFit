...
def _gen_pop(uniprot_id, length, popnames):
	filename = "syn_info_by_protein_mapping/" + uniprot_id + "_info.txt.gz"
	AN = np.load(f"pop_ukbb_gnomad/{uniprot_id}_AN.npy")
	AC = np.load(f"pop_ukbb_gnomad/{uniprot_id}_AC.npy")
	AA_mask = np.load(f"pop_ukbb_gnomad/{uniprot_id}_AA_mask.npy")
...
def main():
	...
	af_dir = "pop_ukbb_gnomad_with_syn/"
	popnames = ["UKBB", "gnomAD_NFE_exome", "gnomAD_NFE_genome"]
	for _, row in df.iterrows():
		uniprot_id = row['UniprotID']
		length = row['Length']
		AN, AC, AA_mask = _gen_pop(uniprot_id, length, popnames)
