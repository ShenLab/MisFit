...
def main():
	...
	for _, row in geneset.iterrows():
		...
		if not os.path.exists("mis_info_by_protein/" + uniprot_id + "_info.txt.gz"):
			continue
		df = pd.read_table("mis_info_by_protein/" + uniprot_id + "_info.txt.gz")
		...
		if len(df) > 0:
			df.to_csv("mis_info_by_protein_mapping/" + uniprot_id + "_info.txt.gz", float_format = "%.4e", sep = "\t", index = False)
