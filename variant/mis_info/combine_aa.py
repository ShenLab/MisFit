...
def main():
	...
	for _, row in geneset.iterrows():
		...
		if not os.path.exists("mis_info_by_protein/" + transcript_name + "_info.txt"):
			continue
		df = pd.read_table("mis_info_by_protein/" + transcript_name + "_info.txt", header = None, names = ['chrom', 'pos', 'ref', 'alt', 'ensembl_pep_pos', 'ref_aa', 'alt_aa', 'gnomad_mu', 'UKBB_AN', 'UKBB_AC'])
		df = df.dropna()
		...
		if len(df_combine) > 0:
			df_combine.to_csv("mis_info_uniprot_ukbb/" + uniprot_id + "_uniaa.txt.gz", float_format = "%.4e", sep = "\t", index = False)
