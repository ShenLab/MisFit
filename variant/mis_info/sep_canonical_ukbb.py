	...
	for line in input_file:
		...
		for csq in csq_all:
			...
			if all_fields is not None:
				...
				if is_missense(all_fields[1]): # missense
					AApos = all_fields[14].split("/")[0]
					refAA = all_fields[15].split("/")[0]
					altAA = all_fields[15].split("/")[1]
					output_file = open(output_dir + trans_id + "_info.txt", "a")
					print(line_split[0], line_split[1], line_split[3], line_split[4], AApos, refAA, altAA, get_content(mu_search), get_content(AN_search), get_content(AC_search, "0"), sep = "\t", file = output_file)
					output_file.close()
