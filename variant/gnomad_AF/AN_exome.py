...
def main(AN_name):
	pos_file = gzip.open("all_pos_alt_gnomad_AF.vcf.gz", "rt")
	region_file = open("exome_regions.txt", "r")
	output_file = open(f"exome_{AN_name}.bed", "w")
	...
	for line in pos_file:
		...
		if region_start > AN_pos:
			continue
		AN_search = re.search(rf"{AN_name}=(\d+)", line_split[7])
		if AN_search:
			AN = AN_search.group(1)
		else:
			continue
