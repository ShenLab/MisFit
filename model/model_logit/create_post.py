...
class PostSelection(tfk.Model):
	...
	def call(self, inputs, d_mean, d_sd, s_mean):
		...
		x = tf.stack([mu, an, ac - an, tf.broadcast_to(s_mean, tf.shape(mu)), tf.broadcast_to(d_mean, tf.shape(mu)), tf.broadcast_to(d_sd, tf.shape(mu))], axis = -1) # (B, L, A, ?)
	...
	def get_s(self, inputs, d_mean, d_sd, s_mean):
		d_post_mean, _ = self.call(inputs, d_mean, d_sd, s_mean)
		s_post_mean = tf.math.sigmoid(d_post_mean) * (s_mean - self.smin) + self.smin
		return s_post_mean
	def sample(self, inputs, d_mean, d_sd, s_mean):
		d_post_mean, d_post_sd = self.call(inputs, d_mean, d_sd, s_mean)
