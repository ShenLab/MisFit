...
class PostSelection(tfk.Model):
	def __init__(self, smin, smax = 0., mean_layer = [10, 10], sd_layer = [10, 10], **kwargs):
	...
	def call(self, inputs, d_mean, d_sd):
		an = tf.math.log(inputs['AN'] + 0.1) # B, L, A
		ac = tf.math.log(inputs['AC'] + 0.1)
		mu = tf.math.log(inputs['mu'] + 1e-10)
		x = tf.stack([mu, an, ac - an, tf.broadcast_to(d_mean, tf.shape(mu)), tf.broadcast_to(d_sd, tf.shape(mu))], axis = -1) # (B, L, A, ?)
	...
	def get_s(self, inputs, d_mean, d_sd):
		d_post_mean, _ = self.call(inputs, d_mean, d_sd)
		s_post_mean = tf.math.sigmoid(d_post_mean) * (self.smax - self.smin) + self.smin
		return s_post_mean
	def sample(self, inputs, d_mean, d_sd):
		d_post_mean, d_post_sd = self.call(inputs, d_mean, d_sd)
		...
		s_post_sample = tf.math.sigmoid(d_post_sample) * (self.smax - self.smin) + self.smin # B, L, A
...
class PostModel(tfk.Model):
	...
	def call(self, inputs, nogene = False):
		d_mean, _, d_sd = self.probmodel.call(inputs)
		s_post_mean = self.post_selection.get_s(inputs, d_mean, d_sd)
		return tf.math.sigmoid(d_mean), s_post_mean
	def get_loss(self, inputs, nogene = False):
		d_mean, _, d_sd = self.probmodel.call(inputs)
		s_post_sample, kl_d = self.post_selection.sample(inputs, d_mean, d_sd)
...
def main():
	model = create_post()
	train_post(model, [2], lr = 5e-4, epochs = 25, patience = 15)
